$(document).ready(function() {var formatter = new CucumberHTML.DOMFormatter($('.cucumber-report'));formatter.uri("Scenario.feature");
formatter.feature({
  "line": 1,
  "name": "Test various functionalities of an application",
  "description": "",
  "id": "test-various-functionalities-of-an-application",
  "keyword": "Feature"
});
formatter.background({
  "line": 4,
  "name": "",
  "description": "",
  "type": "background",
  "keyword": "Background"
});
formatter.step({
  "line": 5,
  "name": "user is on Home page",
  "keyword": "Given "
});
formatter.match({
  "location": "Definitions.natigate_to_home_page()"
});
formatter.result({
  "duration": 9847673223,
  "status": "passed"
});
formatter.scenario({
  "line": 8,
  "name": "Successful registration",
  "description": "",
  "id": "test-various-functionalities-of-an-application;successful-registration",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 7,
      "name": "@Registration"
    }
  ]
});
formatter.step({
  "line": 9,
  "name": "user navigates to sign in page",
  "keyword": "When "
});
formatter.step({
  "line": 10,
  "name": "user clicks enters valid email",
  "keyword": "And "
});
formatter.step({
  "line": 11,
  "name": "user click on create an account",
  "keyword": "And "
});
formatter.step({
  "line": 12,
  "name": "user give all required registration details",
  "keyword": "And "
});
formatter.step({
  "line": 13,
  "name": "user clicks on register",
  "keyword": "And "
});
formatter.step({
  "line": 14,
  "name": "user should be able to register successfully",
  "keyword": "Then "
});
formatter.match({
  "location": "Definitions.user_navigate_to_signin()"
});
formatter.result({
  "duration": 3217847294,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_enter_valid_email()"
});
formatter.result({
  "duration": 423358780,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_click_on_create_an_account()"
});
formatter.result({
  "duration": 102633433,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_give_all_required_registration_details()"
});
formatter.result({
  "duration": 7128248742,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_clicks_on_register()"
});
formatter.result({
  "duration": 1824302240,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_should_be_able_to_register_successfully()"
});
formatter.result({
  "duration": 799660842,
  "status": "passed"
});
formatter.background({
  "line": 4,
  "name": "",
  "description": "",
  "type": "background",
  "keyword": "Background"
});
formatter.step({
  "line": 5,
  "name": "user is on Home page",
  "keyword": "Given "
});
formatter.match({
  "location": "Definitions.natigate_to_home_page()"
});
formatter.result({
  "duration": 9367836803,
  "status": "passed"
});
formatter.scenario({
  "line": 17,
  "name": "Error displayed for invalid email address",
  "description": "",
  "id": "test-various-functionalities-of-an-application;error-displayed-for-invalid-email-address",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 16,
      "name": "@InvalidEmail"
    }
  ]
});
formatter.step({
  "line": 18,
  "name": "user navigates to sign in page",
  "keyword": "When "
});
formatter.step({
  "line": 19,
  "name": "user enters invalid email",
  "keyword": "And "
});
formatter.step({
  "line": 20,
  "name": "user click on create an account",
  "keyword": "And "
});
formatter.step({
  "line": 21,
  "name": "error message should be displayed",
  "keyword": "Then "
});
formatter.match({
  "location": "Definitions.user_navigate_to_signin()"
});
formatter.result({
  "duration": 1858310962,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_enters_invalid_email()"
});
formatter.result({
  "duration": 236540887,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_click_on_create_an_account()"
});
formatter.result({
  "duration": 97006653,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.error_message_should_be_displayed()"
});
formatter.result({
  "duration": 773189657,
  "status": "passed"
});
formatter.background({
  "line": 4,
  "name": "",
  "description": "",
  "type": "background",
  "keyword": "Background"
});
formatter.step({
  "line": 5,
  "name": "user is on Home page",
  "keyword": "Given "
});
formatter.match({
  "location": "Definitions.natigate_to_home_page()"
});
formatter.result({
  "duration": 9641489466,
  "status": "passed"
});
formatter.scenario({
  "line": 24,
  "name": "Validate Summer Dresses Megamenu",
  "description": "",
  "id": "test-various-functionalities-of-an-application;validate-summer-dresses-megamenu",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 23,
      "name": "@SummerDressesMegamenu"
    }
  ]
});
formatter.step({
  "line": 25,
  "name": "user clicks on Dresses",
  "keyword": "When "
});
formatter.step({
  "line": 26,
  "name": "user clicks on Summer Dresses",
  "keyword": "And "
});
formatter.step({
  "line": 27,
  "name": "user navigates Summer Dresses page",
  "keyword": "Then "
});
formatter.match({
  "location": "Definitions.user_clicks_on_Dresses()"
});
formatter.result({
  "duration": 58115268,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_clicks_on_Summer_Dresses()"
});
formatter.result({
  "duration": 3059878911,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_navigates_Summer_Dresses_page()"
});
formatter.result({
  "duration": 761814072,
  "status": "passed"
});
formatter.background({
  "line": 4,
  "name": "",
  "description": "",
  "type": "background",
  "keyword": "Background"
});
formatter.step({
  "line": 5,
  "name": "user is on Home page",
  "keyword": "Given "
});
formatter.match({
  "location": "Definitions.natigate_to_home_page()"
});
formatter.result({
  "duration": 9294108188,
  "status": "passed"
});
formatter.scenario({
  "line": 30,
  "name": "Sort summer dresses by price",
  "description": "",
  "id": "test-various-functionalities-of-an-application;sort-summer-dresses-by-price",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 29,
      "name": "@Sorting"
    }
  ]
});
formatter.step({
  "line": 31,
  "name": "user clicks on Dresses",
  "keyword": "When "
});
formatter.step({
  "line": 32,
  "name": "user clicks on Summer Dresses",
  "keyword": "And "
});
formatter.step({
  "line": 33,
  "name": "user clicks on Sort by price",
  "keyword": "And "
});
formatter.step({
  "line": 34,
  "name": "items should be arranged accordingly",
  "keyword": "Then "
});
formatter.match({
  "location": "Definitions.user_clicks_on_Dresses()"
});
formatter.result({
  "duration": 54374155,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_clicks_on_Summer_Dresses()"
});
formatter.result({
  "duration": 3107225862,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_clicks_on_Sort_by_price()"
});
formatter.result({
  "duration": 5456044626,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.items_should_be_arranged_accordingly()"
});
formatter.result({
  "duration": 753065421,
  "status": "passed"
});
formatter.background({
  "line": 4,
  "name": "",
  "description": "",
  "type": "background",
  "keyword": "Background"
});
formatter.step({
  "line": 5,
  "name": "user is on Home page",
  "keyword": "Given "
});
formatter.match({
  "location": "Definitions.natigate_to_home_page()"
});
formatter.result({
  "duration": 13867124047,
  "status": "passed"
});
formatter.scenario({
  "line": 37,
  "name": "Validate product details in the cart summary",
  "description": "",
  "id": "test-various-functionalities-of-an-application;validate-product-details-in-the-cart-summary",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 36,
      "name": "@CartSummary"
    }
  ]
});
formatter.step({
  "line": 38,
  "name": "user clicks on Dresses",
  "keyword": "When "
});
formatter.step({
  "line": 39,
  "name": "user select a dress",
  "keyword": "When "
});
formatter.step({
  "line": 40,
  "name": "user change the dress color to blue",
  "keyword": "And "
});
formatter.step({
  "line": 41,
  "name": "user add the product to cart",
  "keyword": "And "
});
formatter.step({
  "line": 42,
  "name": "user should see proper Details of the product in the cart summary",
  "keyword": "Then "
});
formatter.match({
  "location": "Definitions.user_clicks_on_Dresses()"
});
formatter.result({
  "duration": 46927281,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_select_a_dress()"
});
formatter.result({
  "duration": 2526147309,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_change_the_dress_color_to_blue()"
});
formatter.result({
  "duration": 119958534,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_add_the_product_to_cart()"
});
formatter.result({
  "duration": 2387305874,
  "status": "passed"
});
formatter.match({
  "location": "Definitions.user_should_see_proper_Details_of_the_product_in_the_cart_summary()"
});
formatter.result({
  "duration": 1059214623,
  "status": "passed"
});
});